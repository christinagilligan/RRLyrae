...
absmag=2.44*np.log10(period)-1.0

logd=(w1mean-absmag+5)/5

distance=10**(logd)


bayestar=BayestarWebQuery(version='bayestar2017')
file = open('Reddening.dat','w')
file.write('#gaiaid parallax sig_parallax w1mean sig_w1 RA Dec G G_BP G_RP Planckred newDistance catalog flag'+'\n')
file.close()


for index in range(3220, len(gaiaid)-1):
    try:
        time.sleep(1)
        ...
        if np.isnan(reddeningValue)==True:
            catalogFlag="Stilism"
            reddeningList=reddening(RA[index],'deg',Dec[index],'deg','icrs')
            nearestDistanceIndex=find_nearest(reddeningList[0],newDistance)
            nearestDistance=reddeningList[0][nearestDistanceIndex]
            reddeningValue=reddeningList[1][nearestDistanceIndex]
            ...
            if reddeningValue==reddeningValueMax and reddeningValue==reddeningValueMin:
                distanceDifference="minimum"
            else:
                distanceDifference="good"

        file = open('Reddening.dat','a')
        file.write(str(gaiaid[index])+' '+str(parallax[index])+' '+str(parallaxError[index])+' '+str(period[index])+' '+str(w1mean[index])+' '+str(w1error[index])+' '+str(RA[index])+' '+str(Dec[index])+' '+str(G[index])+' '+str(B[index])+' '+str(R[index])+' '+str(reddeningValue)+' '+str(newDistance)+' '+str(catalogFlag)+' '+str(distanceDifference)+'\n')
        file.close()
